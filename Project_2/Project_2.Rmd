...
coefsLasso[1:18,]
``` 
Notice that the the lasso model gives a nonzero value for all coefficients, bu as the high values for the same coefficients as forward selection.
...
## b)
The basis functions for the cubic spline with knots $d_1$, $d_2$ and $d_3$ are
$$
\begin{aligned}
& x & h(x,d_1)\\
& x^2 & h(x,d_2) \\
...
# scatterplots 
p1 = ggplot(data=college.train, aes(x=Private, y=Outstate)) + geom_point()  
p2 = ggplot(data=college.train, aes(x=Room.Board, y=Outstate)) + geom_point() 
p3 = ggplot(data=college.train, aes(x=Terminal, y=Outstate)) + geom_point()  
p4 = ggplot(data=college.train, aes(x=perc.alumni, y=Outstate)) + geom_point() 
p5 = ggplot(data=college.train, aes(x=Expend, y=Outstate)) + geom_point()  
p6 = ggplot(data=college.train, aes(x=Grad.Rate, y=Outstate)) + geom_point() 
ggarrange(
  p1, p2, p3,
  p4, p5, p6,
  ncol = 3, 
...
```
From the scatter plots above `Room.Board` and `perc.alumni` seem to have a linear relationship with `Outstate`. Conversely, `Terminal`, `Expend` and to some degree `Grad.Rate` seem to benefit form a non-linear transformation.
...
for (i in 1:10){
  ...
  poly_MSE_errors[i] = mean((pred.poly-college.test$Outstate)^2)
...
```
In the two plots above show regression with polynomials of different degrees.
...
for (i in seq(1, 10, by=2)){
  cat("deg",i,":",poly_MSE_errors[i])
  cat(", deg",i+1,":",poly_MSE_errors[i+1])
  cat("\n")
}
cat("smooth:",test_MSE_smooth)
```
The test MSEs are printed above. I would have expected that polynomials of high order would give high error because of overfitting witch seems to be the case for the polynomial regression. The reason for the high test MSE for the smoothing spline can be similarly explained by the relatively high degree of freedom. 
