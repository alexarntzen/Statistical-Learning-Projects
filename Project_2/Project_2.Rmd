...
coefsLasso[1:18,]
``` 
Notice that the the lasso model gives a nonzero value for all coefficients, but as the high values for the same coefficients as forward selection.
...
## b)
The basis functions for the cubic spline with knots $d_1$, $d_2$ and $d_3$ at the quantiles $q_1$, $q_2$ and $q-3$ are
$$
\begin{aligned}
& b_1(x) = x & b_4(x) = h(x,d_1)\\
& b_2(x) = x^2 & b_5(x) = h(x,d_2) \\
& b_3(x) = x^3 & b_6(x) = h(x,d_3), 
\end{aligned}
$$
where 
$$
...
```
The test MSEs are printed above. I would have expected that polynomials of high order would give high error because of overfitting which seems to be the case for the polynomial regression. The reason for the high test MSE for the smoothing spline can be similarly explained by the relatively high degree of freedom. 
