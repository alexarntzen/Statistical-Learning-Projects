...
MSE.table1 = data.frame(Method = method.Poly , test_MSE = poly_MSE_test)
methods = c("Forward selection", "Lasso", "Smoothing spline","Regression Tree", "Boosting")
MSEs = c(test_MSE_Forward, test_MSE_Lasso, test_MSE_smooth,MSE_regtree, MSE.boost)
MSE.table2 = data.frame(Method=methods ,test_MSE = MSEs)
...
We also see that the linear models performed worse than Boosting. This indicates that the response is not well approximated by a linear model. Still, if interpretability was the highest priority the tree model would probably be the best choice. 
