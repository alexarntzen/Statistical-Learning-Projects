...
\end{equation}
$$
Secondly note that $y_0$ and the training data are independent. In addition $\mathrm{E}[ \epsilon ] \ =\ 0 $. Then expanding equation $\eqref{eq1}$ and using equation $\eqref{eq2}$ the decomposition becomes 
=======
...
\E[ y_{0} -\hat{f}( x_{0})]^{2} & \ =\E[ f( x_{0}) +\epsilon -\hat{f}( x_{0})]^{2}\\
 ...
 & =\Var[ \epsilon ] + [ f( x_{0}) - \E[\hat{f}( x_{0})]]^{2}   + \Var[ \hat{f}( x_{0})].
\end{aligned}
$$
## c)
...
$\pmb{\Var[ \hat{f}( x_{0})]}$ is the **variance** of the prediction at $x_{0}. It is a measure of how much the model will change based on different training data. In general the variance of the model increases when the the flexibility of the model increases. 
...
=======
$[ f( x_{0}) - \E[\hat{f}( x_{0})]]^{2}$ is the **squared bias** at $x_{0}$, also denoted $\mathrm{Bias}(\hat{f}( x_{0}))]^{2}$. It is a measure of much the model differs from the target function $f$ at $x_{0}$. In general a flexible model will have low bias. 
>>>>>>> alexbranch
...
% Er det riktig å ta med feilleddet på slutten ( +0.24106)?
$$
...
Here, the p-value of the interaction terms are also relatively far away from zero, and the F-statistic from the anova table is close to 1, both of which supports our results from $\textbf{2c)}$. In other words, an interaction term between the logarithm of the circumference and the species does not improve the model.
...
<font color = red> Riktig konklusjon? </font>
