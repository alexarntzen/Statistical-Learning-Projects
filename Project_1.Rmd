...
## a)
The dataset consists of 143 rows and 5 columns, which can be seen from running the $str()$-function.
...
To get the separate regression models, we have to change the dummy variable, or the reference category.
```{r, eval=TRUE, echo=TRUE}
d.worm = mutate(d.worm, Gattung = relevel(Gattung, ref = "N"))  # reference category "N"
fit.lmN = lm(log(GEWICHT) ~ log(MAGENUMF) + Gattung, data = d.worm)
summary(fit.lmN)
anova(fit.lmN)

d.worm = mutate(d.worm, Gattung = relevel(Gattung, ref = "Oc"))
fit.lmOc = lm(log(GEWICHT) ~ log(MAGENUMF) + Gattung, data = d.worm)
summary(fit.lmOc)
anova(fit.lmOc)
```

The separate equations where the three different species are the reference categories are thus on the form

$$
\text{log(GEWICHT)} = \hat{\beta_0} + \hat{\beta_1} \text{log(MAGENUMF)} + \hat{\beta_2} \text{Gattung}_1 + \hat{\beta_3} \text{Gattung}_2,
$$
where the subscripts 1 and 2 correspond to the factor levels of Gattung who are not the reference category. These equations are given by

$$
\text{log(GEWICHT)} = -3.13865 + 2.59310 \cdot \text{log(MAGENUMF)} + 0.07327 \cdot \text{Gattung}_N - 0.06149 \cdot \text{Gattung}_{Oc} \quad \text{Ref. = "L"} \\

\text{log(GEWICHT)} = -3.06538 + 2.59310 \cdot \text{log(MAGENUMF)} - 0.07327 \cdot \text{Gattung}_L - 0.13476 \cdot \text{Gattung}_{Oc} \quad \text{Ref. = "N"} \\

\text{log(GEWICHT)} = -3.20013 + 2.59310 \cdot \text{log(MAGENUMF)} + 0.13476 \cdot \text{Gattung}_N + 0.06149 \cdot \text{Gattung}_{L} \quad \text{Ref. = "Oc"}
$$
MÅ FINNE UT OM DETTE ER RIKTIG TOLKNING AV SPØRSMÅLET!!
...
<font color = red>SJEKK DENNE!!</font>
...
Hence we see that the odds will increase with a factor of $\beta_1$  if player 1 has one more ace, $x_{i1}' = x_{i1} + 1 $. As $\beta_1 = 0.36338$ we know that one more ace for player 1 will increase the likelihood for player 1 winning the match with a factor of $e^{0.36338} \approx 1.438$, in our model.
...
We see that the term outside the exponent cancels because of the assumption that $\Sigma_0 = \Sigma_1 = \Sigma$, hence we take $\log$ on both side to get:
...
When we derived the decision boundary for LDA we assumed that each class have equal covariance matrix. For QDA we do not assume this and hence the quadratic terms of $x$ does not cancel and we obviously get a decision boundary that is quadratic.
```{r}
...
testPlotMale 
```
"The probability of chd for a male with a sbp=140 is .
...
```
from the probabilies we get a standard error of r stdError.A $95\%$ confidence interval for the the estimator of $P(\mathrm{chd} | \mathrm{sex= male},\mathrm{sbp=40})$ is [ `
r low ,r high]. 
