...
$\pmb{\mathrm{Var}[ \epsilon ]}$ is the **irreducible error**. It is independent of the model and cannot be reduced. 
...
$[ f( x_{0}) - \mathrm{E}[\hat{f}( x_{0})]]^{2}$ is the **squared bias** at $x_{0}$, also denoted $\mathrm{Bias}(\hat{f}( x_{0}))]^{2}$. It is a measure of much the model differs from the target function $f$ at $x_{0}$. In general a flexible model will have low bias. 
